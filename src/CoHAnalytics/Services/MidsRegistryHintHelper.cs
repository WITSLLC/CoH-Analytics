...
internal static class MidsRegistryHintHelper
{
    ...
    private static IEnumerable<string> EnumerateHive(RegistryKey hive, string keyPath)
    {
        ...
        foreach (var subKeyName in uninstallKey.GetSubKeyNames())
        {
            ...
            if (!LooksLikeMidsEntry(displayName))
            {
                continue;
            }
    ...
    private static bool LooksLikeMidsEntry(string? displayName)
    {
        ...
        return displayName.Contains("midsreborn", StringComparison.OrdinalIgnoreCase)
               || displayName.Contains("mids reborn", StringComparison.OrdinalIgnoreCase);
    }
