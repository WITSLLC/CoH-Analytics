...
internal static class HomecomingPowersInspirationDiscoveryReader
{
    private static readonly UTF8Encoding StrictUtf8 = new(false, true);

    internal static IReadOnlyList<HomecomingInspirationDiscoveryRecord> ReadInspirations(byte[] data)
    {
        ...
        try
        {
            ...
            var inspirations = new List<HomecomingInspirationDiscoveryRecord>();
            ...
            for (var index = 0; index < recordCount; index++)
            {
                ...
                HomecomingInspirationDiscoveryRecord parsed;
                try
                {
                ...
                if (!parsed.SourceId.StartsWith("Inspirations.", StringComparison.Ordinal))
                {
                    continue;
                }

                Require(sourceIds.Add(parsed.SourceId),
                    $"Inspiration source ID '{parsed.SourceId}' is duplicated.");
                inspirations.Add(parsed);
            }
    ...
    internal static HomecomingInspirationDiscoveryRecord? TryGetInspiration(byte[] data, string sourceId)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(sourceId);
        return ReadInspirations(data).FirstOrDefault(
        ...
        HomecomingPowerLayout layout)
    {
        ...
        for (var index = 0; index < sampleCount; index++)
        {
            ...
            try
            {
                ...
                if (parsed.SourceId.StartsWith("Inspirations.", StringComparison.Ordinal)
                    && parsed.DisplayNameMessageKey.Length > 0)
                {
                    score++;
                }
            }
    ...
    private static HomecomingInspirationDiscoveryRecord ParseRecord(
        ...
        HomecomingPowerLayout layout)
    {
        ...
        return new HomecomingInspirationDiscoveryRecord(
            sourceId,
...
internal sealed record HomecomingInspirationDiscoveryRecord(
    string SourceId,
