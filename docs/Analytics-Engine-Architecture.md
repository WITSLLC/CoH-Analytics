...
MetricComparison
  ...
  PercentDelta      Metric<long>       // hundredths of a percent; 10_000 = 100%
  PercentagePoint   Metric<long>       // hundredths of a percentage point; 500 = 5.00 pp
  State             ComparisonState    // Comparable | Incompatible | Unavailable | Partial
```

Rules:
- **Raw totals vs rates** are distinct (§14): observed totals compare directly even when durations differ; duration equality is reported as context and never normalizes totals. Rates compare only when their numerator and denominator semantics match exactly. No normalization or replay occurs in Slice 11.
- **Zero-baseline**: `PercentDelta` from a zero left has no value (never ∞) and carries the typed reason `ZeroBaseline`; the absolute comparison remains `Comparable`.
- **Denominator/semantic/attribution mismatch**: gate per field on the persisted semantic and policy facts. Current semantic version 3 is the only accepted analytical contract. Attribution-policy mismatch blocks attribution fields while compatible non-attribution totals remain usable. The comparison engine does not replay or reclassify either source.
- **Partial semantics**: `Incomplete` source metrics may produce explicitly `Partial` deltas. Overall compatibility is `PartiallyComparable` when a usable comparison also has material coverage, build-context, attribution-policy, legacy, duplicate-key, or unavailable-axis limitations. An isolated incompatible metric does not suppress compatible raw totals.
- `NotCaptured` on either side ⇒ `Unavailable` (never treated as 0 difference).
- Missing keyed rows become observed zero only where the parent breakdown proves complete. Duplicate stable keys remain ambiguous and never merge by first, last, or sum. Internal overflow buckets are distinct from literal surfaced names.
- Percentage arithmetic uses `Int128`, divides toward zero, and returns typed `ArithmeticOverflow` rather than throwing when a result cannot fit the stored signed `long` representation.
...
- Slice 12 in-memory `CombatPipelineDiagnostics` observes classified physical line occurrences plus optional later-stage results. Its line counters can overlap and are distinct from logical post-dedup event counts. It is not persisted in `coverage.json` and does not bump `SegmentSchemaVersion`. `IsCanonicalCombatUnparsed` is limited to failed lines in the canonical domain: a recognized grammar or a known canonical amount frame, including one outer pet-prefix strip. Ordinary prose is excluded. Legacy `IsCombatShapedUnparsed` retains its existing broader `CombatEvent`-mapping definition. Diagnostics never change parse, totals, attribution, persistence, or comparison.
...
- **Objective**: `ComparisonEngine` typed deltas (absolute/percent/percentage-point), comparable/partial/incompatible/unavailable states, and per-field semantic-version/denominator/attribution/coverage guards over existing projections.
...
- **Depends on**: 10.
- **Slice 11 implementation notes**: `ComparisonEngine` compares two `AnalyticalProjectionView` (or raw `CombatAnalyticsProjection`) values. Inputs may be live, durable historical, or legacy-adapted. The engine never calls `CombatEngine`, parser/dedup, attribution, `SegmentStore`, or the current catalog/build. `AnalyticsSemanticVersion` stays `3`; both sides must be semantic 3 or the result is `IncompatibleSemanticVersion` with no numeric deltas. Grammar/dedup version differences are not automatic incompatibility under the same semantic contract. Delta convention is Right − Left. `PercentDeltaHundredths` is (Right − Left) / |Left| as hundredths of a percent (10_000 = 100%); division truncates toward zero, a zero left baseline has typed reason `ZeroBaseline`, and unrepresentable deltas have typed reason `ArithmeticOverflow`. Accuracy uses percentage-point delta (`HitRatePercentagePointDeltaHundredths`, 500 = +5.00 pp), not relative percent. Raw totals compare when both metrics are `Available` (or `Partial` when `Incomplete`); rates compare only with identical `RateDenominatorKind`. Wall-clock DPS vs tracked-pause/active is `Incompatible`. Powers match on `(Direction, Scope, normalized pet name, PowerName, IsOverflow)` with no display-name fuzzy matching; player, pet, literal-name, and overflow rows stay distinct. Duplicate stable power keys are retained as ambiguous candidates and produce no numeric merge. Missing damage-type rows are observed zero only when both type breakdowns are `Available`; incomplete type coverage does not invent zero. Target cardinality is exact only when both distinct-target metrics are `Available` without lower-bound/overflow. Defeat zero is not inferred from unrelated combat; positive direct evidence and legacy observation fields establish availability. Build comparison exposes `SameManifestHash` / `SameCatalogFingerprint` as facts, not causes. Missing build or limited source coverage makes an otherwise usable overall result `PartiallyComparable`. Attribution compares policy-1 summaries only; policy mismatch is field-incompatible without rerunning attribution, and policy-1 `Correlated` remains unsupported. Legacy observations remain lower-fidelity: only mapped `Available` axes (damage, accuracy, defeats, wall-clock duration) compare; other dimensions stay `Unavailable`. No ranking, winner, or causal language. Slice 12 is diagnostics/fixtures, not Compare UI.
...
- **Depends on**: 1–11.
- **Slice 12 implementation notes**: Diagnostics observe the pipeline; they do not change parse, dedup, totals, attribution, persistence, or comparison. `CombatPipelineDiagnostics` is an on-demand in-memory report assembled by `CombatPipelineDiagnosticsFactory` from classified parser events plus optional `DedupDiagnostics`, `CombatAnalyticsProjection`, and `SegmentCoverageDescriptor`; it is not automatically collected on the live per-event path. Existing `GameplaySessionDiagnostics` JSONL remains operational session/queue logging and has a separate responsibility. The report is **not** written into `coverage.json` (`SegmentSchemaVersion` remains 1). Persisted coverage stays the Slice 9 truncation + lossless-replay matrix. Line counters count supplied physical occurrences and can overlap; they do not claim logical post-dedup cardinality. `IsCanonicalCombatUnparsed` counts failed recognized grammars and known canonical amount frames, after at most one outer pet-prefix strip. Ordinary prose is not a canonical candidate. It is distinct from legacy `IsCombatShapedUnparsed`, whose existing broader definition uses `CombatEvent` mapping. Pet-prefixed canonical successes are not legacy combat-shaped misses on the unstripped body. Unparsed samples are bounded (8 × 160 chars), path-sanitized before truncation so supported raw path forms and their directory/filename fragments are not retained, and observed channels are likewise path-sanitized, ordinally sorted, and bounded (16 × 64 chars). `CanonicalSemanticDigest` uses length-prefixed binary encoding and SHA-256 as a CI regression canary only. It hashes ordered canonical family/grammar, full actor/target and pet-rollup identity, surfaced power, amount/unit, damage type, delivery, outcome/roll, source channel/timestamp, mirror class, facets/status/lifecycle, and duplicate-presence semantics. It excludes event/parser sequence, runtime GUIDs/source identity, observation times, byte offsets, and classification-rule text. It is not Segment identity, dedup identity, a persistence contract, or a semantic-version substitute. Coverage flags, replay matrix, dedup decisions, and attribution histograms are copied from supplied authoritative state; policy-1 `Correlated` remains unused. Target overflow is diagnosed via `IsOverflow`, never by the literal name `Other`. Diagnostics expose the existing analytics, grammar, dedup, attribution, Segment, and spine version axes without defining a diagnostics version. Fixture corpus: `Fixtures/Combat/max-channel-2026-09-12.tsv`, `power-lifecycle-2026-09-12.tsv`, and `slice12-negative-2026-08-04.tsv`. Grammar-match and Slice 2–5A independent goldens already cover every `CombatGrammarId` exactly once; Slice 12 re-asserts that union. Ambiguity stays ambiguity. The frozen 12-slice analytical engine sequence is complete; Compare/Historical/HTML UI remain deferred.
...
- Header listing is metadata/coverage only. Aggregate load uses the persisted cube. Spine load is coverage-first and optional. Legacy v1/v2 observations map missing axes to `NotCaptured`. Historical UI, HTML, and rebake remain unimplemented.

**Revision 10 — Slice 11 comparison engine.**
- Named `ComparisonEngine`, `MetricComparison<T>`, `AnalyticalComparison`, `ComparisonState` (`Comparable` / `Incompatible` / `Unavailable` / `Partial`), and `AnalyticalComparisonCompatibility`. `AnalyticsSemanticVersion` remains `3`; comparison-only result types are not persisted.
- Comparison is a pure function of two projections. Absolute delta is Right − Left; percent uses a nonzero left baseline; rates compare only with matching denominators; semantic-version mismatch blocks numeric comparison. No Compare UI, HTML, ranking, or rebake.
